...
config_merge() {
        ...
        if [[ "$overwrite" == true ]]; then
                ...
                # Preserve the user's file, then append only the source lines
                # that are not already present (whole-line, fixed-string match).
                cp "$user_file" "$merged_file"

                local line
                while IFS= read -r line; do
                        if ! grep -qxF -- "$line" "$user_file" 2>/dev/null; then
                                printf '%s\n' "$line" >>"$merged_file"
                                ((changes++)) || true
                        fi
                done <"$source_file"
